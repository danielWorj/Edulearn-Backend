...
public class RepetitionControllerImpl implements RepetitionController {
    ...
    private static String folderFileProfil = System.getProperty("user.dir")+"/src/main/resources/templates/educia/public/assets/file/photo"; //chemin a déinir

    @Override
    public ResponseEntity<ServerResponse> createNewOffer(String offre, MultipartFile photo , MultipartFile cni) throws IOException {
        OffreDTO offreDTO = objectMapper.readValue(offre, OffreDTO.class);
        //CREATION DU PARENT

        Parent parent = new Parent();

        parent.setNomComplet(offreDTO.getNomComplet());
        parent.setTelephone(offreDTO.getTelephone());
        parent.setEmail(offreDTO.getEmail());
        parent.setPassword(offreDTO.getPassword());
        parent.setLocalisation(offreDTO.getLocalisation());
        parent.setProfession(offreDTO.getProfession());
        parent.setPhoto(offreDTO.getNomComplet());
        parent.setStatus(false);
        parent.setRole(3);


        Parent parentCreated = this.parentRepository.save(parent); //Parent qui vient d'etre cree

        System.out.println("Le parent qui vient d'etre cree:"+ parentCreated.toString());
        //Offre proprement dite

        Offre offreDB = new Offre();
        offreDB.setBio(offreDTO.getBio());
        ...
        for (Integer idMatiere : offreDTO.getMatieres()) {
            Matiere matiere = this.matiereRepository.findById(idMatiere)
                    .orElseThrow(() -> new EntityNotFoundException(
                            "Matiere introuvable : " + idMatiere)); // Gérer le cas où la matière n'existe pas

            MatiereNewOffre matiereOffre = new MatiereNewOffre();
            matiereOffre.setOffre(offreCreated);
            matiereOffre.setMatiere(matiere);
            this.matiereNewOffreRepository.save(matiereOffre);
        }

        //TODO : Sauvegarder les fichiers (photo et cni) dans un stockage de fichiers ou une base de données, et associer les chemins ou les références aux entités correspondantes (Parent ou Offre).
        String fileName = "UNKNOW FILE";
        if (!photo.isEmpty()){
            //S'il n'y a pas de fichier
            fileName = offreDTO.getNomComplet(); // le fichier prend le nom du parent
            //Sauvegarde du fichier en local
            System.out.println("le nom du fichier "+ fileName);
            Path path = Paths.get(folderFileProfil,fileName);
            ...
            System.out.println("photo de profil enregistre en base de donnee");
        }

        String fileNameCNI ="UNKNOW CNI";
        if (!cni.isEmpty()){
            //S'il n'y a pas de fichier
            fileNameCNI = offreDTO.getNomComplet(); // le fichier prend le nom du client

            // Sauvegarde du fichier en local
            System.out.println("le nom du fichier "+ fileNameCNI);
            Path path = Paths.get(folderFileCNI,fileNameCNI);
            cni.transferTo(path);
            ...
            System.out.println("cni enregistre en base de donnee");
        }
